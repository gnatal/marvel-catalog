...
export default async function Comics({
  ...
  const data = await getAllComics(letterFilter as string, ITEMS_PER_PAGE, offset);
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="container mx-auto px-4 mb-6">
        <div className="text-gray-400 font-mono text-sm">
          <span className="text-cyan-400">{comics.length}</span> comics found
          {letterFilter && (
            <span>
              {" "}
              {`starting with '`}
              <span className="text-fuchsia-500">{letterFilter}</span>
              {`'`}
            </span>
          )}
        </div>
        ...
        {comics.length > 0 ? (
          ...
          <div className="text-center py-16">
            <div className="inline-block border border-gray-800 p-6 rounded">
              ...
              <p className="text-gray-400 font-mono text-sm">{`No comics found starting with '{letterFilter}'`}</p>
              <Link
                href="/comics"
